...
es = Elasticsearch(['http://34.73.47.238:9200'])
# http://34.73.47.238
   ...
   # res[]
    for hit in resIndivId['aggregations']['indivs']['buckets']:
        ...
        for i in range(len(resPerIndividual['hits']['hits'])-1):
            ...
            current = int(hit["_source"]["other_income_amt"])
            next_one = int(hit_next["_source"]["other_income_amt"])
            
            print('current', current, ' next', next_one,' per diff: ', (next_one - current ) / current * 100)
